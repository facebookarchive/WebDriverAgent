...
+ (NSDictionary *)routeHandlers
{
  ...
  @{
    @"GET@/session/:sessionID/orientation" : ^(FBRouteRequest *request, FBRouteResponseCompletion completionHandler) {
      completionHandler(FBResponseDictionaryWithStatus(FBCommandStatusNoError, [self.class interfaceOrientationForApplication:request.session.application]));
    },
    @"POST@/session/:sessionID/orientation" : ^(FBRouteRequest *request, FBRouteResponseCompletion completionHandler) {
      if ([self.class setDeviceOrientation:request.arguments[@"orientation"] forApplication:request.session.application]) {
