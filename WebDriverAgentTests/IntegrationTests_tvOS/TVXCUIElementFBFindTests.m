...
@interface TVXCUIElementFBFindTests : FBTVIntegrationTestCase
@property (nonatomic, strong) XCUIElement *testedView;
@end

@implementation TVXCUIElementFBFindTests
...
- (void)setUp
{
  ...
  self.testedView = self.testedApplication.otherElements[@"MainView"].otherElements.firstMatch;
...
- (void)testDescendantsWithClassChain
{
  ...
  matchingSnapshots = [self.testedApplication fb_descendantsMatchingClassChain:@"XCUIElementTypeWindow/*/*/XCUIElementTypeButton" shouldReturnAfterFirstMatch:NO];
...
{
  NSArray<XCUIElement *> *matchingSnapshots;
  matchingSnapshots = [self.testedApplication fb_descendantsMatchingClassChain:@"XCUIElementTypeWindow/*/*/XCUIElementTypeButton[`label BEGINSWITH 'A'`]" shouldReturnAfterFirstMatch:NO];
...
{
  NSArray<XCUIElement *> *simpleQueryMatches = [self.testedApplication fb_descendantsMatchingClassChain:@"XCUIElementTypeWindow/*/*/XCUIElementTypeButton[`label BEGINSWITH 'A'`]" shouldReturnAfterFirstMatch:NO];
...
- (void)testClassChainWithDescendantPredicate
{
  ...
  NSArray<XCUIElement *> *predicateQueryMatches = [self.testedApplication fb_descendantsMatchingClassChain:@"XCUIElementTypeWindow/*/*/*[$type == 'XCUIElementTypeButton' AND label BEGINSWITH 'At'$]" shouldReturnAfterFirstMatch:NO];
...
- (void)testSingleDescendantWithComplexIndirectClassChain
{
  ...
  XCTAssertEqualObjects(queryMatches.lastObject.label, @"Attributes");
}
...
{
  NSArray<XCUIElement *> *matchingSnapshots;
  matchingSnapshots = [self.testedApplication fb_descendantsMatchingClassChain:@"XCUIElementTypeWindow[`name != 'bla'`]/*/*/XCUIElementTypeButton[`label BEGINSWITH \"A\"`][1]" shouldReturnAfterFirstMatch:NO];
  XCTAssertEqual(matchingSnapshots.count, 1);
  XCTAssertEqualObjects([matchingSnapshots firstObject].label, @"Alerts");
}
...
@interface TVXCUIElementFBFindTests_ScrollPage : FBTVIntegrationTestCase
@end
@implementation TVXCUIElementFBFindTests_ScrollPage
...
{
  XCUIElement *table = self.testedApplication.tables.firstMatch;
  NSArray<XCUIElement *> *matches = [table fb_descendantsMatchingClassChain:@"XCUIElementTypeCell" shouldReturnAfterFirstMatch:NO];
  XCTAssertEqual(matches.count, 100);
  XCTAssertEqual([matches firstObject].elementType, XCUIElementTypeCell);
}
